...
typedef struct crayon_sprite_array{
	uint16_t *draw_pos;			//Width then Height extracted from anim/frame data,
										//Each group of 2 is for one sub-texture
	uint8_t *frame_coords_map;	//Contains element ids for group of 4 frame_coords (why not 2?)
										//and uses that for drawing
	uint16_t *frame_coords;		//Each group of 4 elements is one frame of an animation
	uint32_t *colour;			//For poly mode this dictates the rgb and alpha of a polygon
