...
#include <dc/spu.h> //Might be redundant
#include <dc/sound/sound.h> //Might be redundant
...
int main(){
    ...
    //Mount romdisk
    //memory_mount_romdisk("/cd/sound.img", "/sound");
    sfxhnd_t Sound_T = snd_sfx_load("/rd/Sound_009.wav");
    //fs_romdisk_unmount("/sound");
	...
	controller = maple_enum_type(0, MAPLE_FUNC_CONTROLLER);	//Reads the first plugged in controller
        ...
        if(st->buttons & (1 << 2) && (!(previous_buttons & (1 << 2)))){	//When the button is pressed, play the sound
